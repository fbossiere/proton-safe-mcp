...
begin
  ...
  if not UninstallSilent then
    EraseChosen := MsgBox(ExpandConstant('{cm:EraseData}'), mbConfirmation, MB_YESNO or MB_DEFBUTTON2) = IDYES;
end;
...
begin
  ...
  begin
    { Missing binaries do not establish that the client connection is gone. Repair
      or explicitly choose software-only removal instead of silently stranding it. }
    Result := 1;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    if MsgBox(ExpandConstant('{cm:RemovalFailed}'), mbError, MB_RETRYCANCEL or MB_DEFBUTTON2) = IDRETRY then
      Continue;
    ...
    if MsgBox(ExpandConstant('{cm:RemoveAnyway}'), mbConfirmation, MB_YESNO or MB_DEFBUTTON2) = IDYES then
      Exit;
