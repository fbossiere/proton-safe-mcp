...
def test_a_hanging_command_is_stopped_by_its_deadline(tmp_path):
    ...
    result = run_command([sys.executable, str(script)], timeout=1.0)
...
def test_a_flooding_command_cannot_exhaust_memory(tmp_path):
    ...
    result = run_command([sys.executable, str(script)], timeout=30.0)
...
def test_arguments_never_reach_a_shell(tmp_path):
    ...
    result = run_command([sys.executable, str(script), f"; touch {marker}"])
...
def test_only_real_executable_files_become_candidates(tmp_path, make_executable):
...
def test_a_non_x86_or_non_linux_system_is_blocked(monkeypatch):
    monkeypatch.setattr(posix_services.platform, "system", lambda: "Darwin")

    assert inventory.system_check().status == "fail"

    monkeypatch.setattr(posix_services.platform, "system", lambda: "Linux")
    monkeypatch.setattr(posix_services.platform, "machine", lambda: "aarch64")
...
def test_the_validated_distribution_passes_and_another_linux_only_warns(monkeypatch, tmp_path):
    monkeypatch.setattr(posix_services.platform, "system", lambda: "Linux")
    monkeypatch.setattr(posix_services.platform, "machine", lambda: "x86_64")
    monkeypatch.setattr(
        posix_services,
        "_os_release",
        lambda: {"ID": "ubuntu", "VERSION_ID": "24.04", "NAME": "Ubuntu"},
    )
    ...
    monkeypatch.setattr(
        posix_services,
        "_os_release",
        lambda: {"ID": "debian", "VERSION_ID": "12", "NAME": "Debian"},
    )
...
def test_a_detected_bridge_application_passes(monkeypatch, tmp_path, make_executable):
    bridge = make_executable(tmp_path, "protonmail-bridge")
    monkeypatch.setattr(inventory.shutil, "which", lambda _name: None)
    monkeypatch.setattr(inventory.shutil, "which", lambda _name: str(bridge))
...
def test_an_unreadable_os_release_does_not_crash_the_check(monkeypatch):
    monkeypatch.setattr(
        posix_services.Path, "read_text", lambda _self, **_kwargs: (_ for _ in ()).throw(OSError())
    )

    assert posix_services._os_release() == {}
