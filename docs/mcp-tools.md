...
Both draft paths store the body twice: the confirmed `body_text` as `text/plain`, plus an
equivalent `text/html` alternative. Proton opens a `text/plain`-only draft in the composer's
**Plain text** mode, so the HTML alternative is what keeps the draft in Proton's default
**Normal** rich-text mode.
