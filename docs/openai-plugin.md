...
`proton-safe-mcp==2.1.2` release with `uvx`. ChatGPT desktop, Codex CLI, and the Codex IDE
...
systemd-run --user --wait --pipe \
  uvx --from proton-safe-mcp==2.1.2 proton-safe-mcp doctor
```
...
3. Choose **STDIO** and configure `uvx` with these arguments:
   ...
   ```text
   --from proton-safe-mcp==2.1.2 proton-safe-mcp serve
   ```
