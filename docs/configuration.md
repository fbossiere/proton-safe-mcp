...
| `PROTON_MCP_STATE_DIR` | `~/.local/state/proton-safe-mcp` | local directory | Private attachment staging state |
...
- Draft confirmation: explicit client assertion of the exact user-confirmed content.
- Sending: unavailable. There is no SMTP client and no send tool.
...
The server creates the state directory and its `uploads` child with mode `0700`. State files use mode `0600` and are opened defensively to avoid following symbolic links.

Uploaded attachment bytes are temporary files, removed once a draft consumes them or once `PROTON_MCP_UPLOAD_TTL_SECONDS` elapses. Draft content is never written to the state directory.

Versions before this release also created an `approvals` child. It is no longer used and can be deleted.
