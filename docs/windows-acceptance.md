...
`packaging/windows/innosetup.lock`. Its digest was verified against the publisher’s immutable release attestation on
14 September 2026. The build also checks the Authenticode publisher before execution.
...
W01 still needs an interactive Windows 11 test. The CI runner uses Windows Server and
creates a disposable standard account for silent installation, repair and uninstall. It
also verifies that elevated installation is refused. These checks do not qualify a real
Windows 11 desktop, its UAC interaction, Bridge or an AI client.
...
1. **Settle the unknowns before finishing the installer.** Credential Manager with local
   ...
   Windows client; activation in that client. Produce evidence for each. The Inno Setup compiler is already pinned and verified
   in `packaging/windows/innosetup.lock`.
